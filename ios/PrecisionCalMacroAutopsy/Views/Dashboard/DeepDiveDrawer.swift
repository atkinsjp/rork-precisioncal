...
struct DeepDiveDrawer: View {
    ...
    private var handle: some View {
        HStack {
            Spacer()
            Capsule()
                .fill(PrecisionCalMacroAutopsyTheme.textTertiary.opacity(0.5))
    ...
    private var densityCard: some View {
        GlassCard {
            HStack(spacing: 16) {
                ZStack {
                    Circle()
                        .stroke(PrecisionCalMacroAutopsyTheme.glassStroke.opacity(0.5), lineWidth: 6)
                        .frame(width: 56, height: 56)
                    Circle()
                        .trim(from: 0, to: CGFloat(nutrientDensity) / 100)
                        .stroke(
                            LinearGradient(colors: [PrecisionCalMacroAutopsyTheme.sage, PrecisionCalMacroAutopsyTheme.terracotta],
...
private struct HazardRow: View {
    ...
    private var color: Color {
        switch hazard.kind {
        case "allergy": return PrecisionCalMacroAutopsyTheme.terracotta
        case "additive": return PrecisionCalMacroAutopsyTheme.fatColor
        default: return PrecisionCalMacroAutopsyTheme.terracottaDeep
        }
    }
