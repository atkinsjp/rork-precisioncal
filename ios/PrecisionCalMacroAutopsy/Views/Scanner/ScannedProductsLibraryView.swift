...
struct ScannedProductsLibraryView: View {
    ...
    private var statsHeader: some View {
        GlassCard {
            HStack(spacing: 0) {
                statBlock(label: "Items", value: "\(products.count)", color: PrecisionCalMacroAutopsyTheme.terracotta)
                Divider().frame(height: 36).background(PrecisionCalMacroAutopsyTheme.glassStroke)
                statBlock(label: "Low risk", value: "\(products.filter { $0.riskLevel == "low" }.count)", color: PrecisionCalMacroAutopsyTheme.sage)
                Divider().frame(height: 36).background(PrecisionCalMacroAutopsyTheme.glassStroke)
                statBlock(label: "High risk", value: "\(products.filter { $0.riskLevel == "high" }.count)", color: PrecisionCalMacroAutopsyTheme.terracotta)
            }
            .padding(.vertical, 16)
        }
    }
                ...
                ForEach(RiskFilter.allCases) { f in
                    ...
                    Button {
                        ...
                        Text(f.rawValue)
                            ...
                            .foregroundStyle(isOn ? .white : PrecisionCalMacroAutopsyTheme.textSecondary)
                            .padding(.vertical, 8)
                            .padding(.horizontal, 14)
                            .background {
                                Capsule()
                                    .fill(isOn ? PrecisionCalMacroAutopsyTheme.textPrimary : Color.white.opacity(0.5))
