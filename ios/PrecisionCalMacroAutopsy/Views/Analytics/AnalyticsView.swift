            ...
            HStack(spacing: 4) {
                ForEach(AnalyticsTab.allCases) { t in
                    Button {
                        ...
                        .foregroundStyle(tab == t ? .white : PrecisionCalMacroAutopsyTheme.textSecondary)
                        ...
                        .background {
                            if tab == t {
                                RoundedRectangle(cornerRadius: 16, style: .continuous)
                                    ...
                                    .shadow(color: PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.3), radius: 8, y: 4)
                            }
                        }
                    }
                    .buttonStyle(.plain)
        ...
        HStack(spacing: 8) {
            ForEach(AnalyticsRange.allCases) { r in
                Button {
                    ...
                    Text(r.rawValue)
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundStyle(range == r ? .white : PrecisionCalMacroAutopsyTheme.textSecondary)
                        .padding(.vertical, 8)
                        .padding(.horizontal, 14)
                        .background {
                            Capsule()
                                .fill(range == r ? PrecisionCalMacroAutopsyTheme.textPrimary : Color.white.opacity(0.5))
                                .overlay(
                                    Capsule().stroke(PrecisionCalMacroAutopsyTheme.glassStroke, lineWidth: 1)
                                )
                        }
                }
                .buttonStyle(.plain)
...
enum Nutrient: String, CaseIterable, Identifiable {
    ...
    var color: Color {
        switch self {
        case .calories: PrecisionCalMacroAutopsyTheme.terracotta
        case .protein: PrecisionCalMacroAutopsyTheme.proteinColor
        case .carbs: PrecisionCalMacroAutopsyTheme.carbColor
        case .fat: PrecisionCalMacroAutopsyTheme.fatColor
        case .fiber: PrecisionCalMacroAutopsyTheme.sage
            ...
            HStack(spacing: 8) {
                ForEach(Nutrient.allCases) { n in
                    ...
                    Button {
                        ...
                        .foregroundStyle(isOn ? PrecisionCalMacroAutopsyTheme.textPrimary : PrecisionCalMacroAutopsyTheme.textTertiary)
                        .padding(.vertical, 8)
                        ...
                        .background {
                            Capsule()
                                ...
                                .overlay(
                                    Capsule().stroke(isOn ? n.color.opacity(0.6) : PrecisionCalMacroAutopsyTheme.glassStroke, lineWidth: 1)
                                )
                        }
                    }
                    .buttonStyle(.plain)
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                .chartXAxis {
                    AxisMarks(values: .automatic(desiredCount: 4)) { value in
                        AxisGridLine().foregroundStyle(PrecisionCalMacroAutopsyTheme.glassStroke.opacity(0.5))
                ...
                .chartYAxis {
                    AxisMarks(position: .leading) { _ in
                        AxisGridLine().foregroundStyle(PrecisionCalMacroAutopsyTheme.glassStroke.opacity(0.4))
                        AxisValueLabel()
                            .font(.system(size: 10, weight: .medium))
                            .foregroundStyle(PrecisionCalMacroAutopsyTheme.textTertiary)
                    }
...
struct WaterAnalyticsSection: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    ...
                    Text("Target \(Int(target)) ml")
                        ...
                        .foregroundStyle(PrecisionCalMacroAutopsyTheme.hydrationColor)
                }
                ...
                Chart {
                    ForEach(dailyTotals) { p in
                        ...
                        .foregroundStyle(
                            LinearGradient(
                                colors: [PrecisionCalMacroAutopsyTheme.hydrationColor.opacity(0.5), PrecisionCalMacroAutopsyTheme.hydrationColor.opacity(0.05)],
                    ...
                    RuleMark(y: .value("Target", target))
                        .foregroundStyle(PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.7))
    ...
    private var statsRow: some View {
        HStack(spacing: 12) {
            statTile(label: "Daily avg", value: "\(Int(avg))", unit: "ml", color: PrecisionCalMacroAutopsyTheme.hydrationColor, delay: 0.2)
            statTile(label: "Streak", value: "\(streak)", unit: streak == 1 ? "day" : "days", color: PrecisionCalMacroAutopsyTheme.terracotta, delay: 0.3)
            statTile(label: "Logs", value: "\(filtered.count)", unit: "sips", color: PrecisionCalMacroAutopsyTheme.sage, delay: 0.4)
        }
    }
...
struct WeightAnalyticsSection: View {
                ...
                if points.isEmpty {
                    ...
                    Chart {
                        ForEach(points) { p in
                            ...
                            .foregroundStyle(
                                LinearGradient(
                                    colors: [PrecisionCalMacroAutopsyTheme.terracotta, PrecisionCalMacroAutopsyTheme.fatColor],
    ...
    private var statsRow: some View {
        HStack(spacing: 12) {
            tile("Change", value: String(format: "%+.1f", change), unit: "lb", color: change <= 0 ? PrecisionCalMacroAutopsyTheme.sage : PrecisionCalMacroAutopsyTheme.terracotta, delay: 0.2)
            tile("Lowest", value: filtered.isEmpty ? "—" : String(format: "%.1f", minW), unit: "lb", color: PrecisionCalMacroAutopsyTheme.sage, delay: 0.3)
            tile("Highest", value: filtered.isEmpty ? "—" : String(format: "%.1f", maxW), unit: "lb", color: PrecisionCalMacroAutopsyTheme.fatColor, delay: 0.4)
        }
    }
