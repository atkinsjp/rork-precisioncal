...
struct ShoppingListView: View {
    ...
    private var statsHeader: some View {
        GlassCard {
            HStack(spacing: 0) {
                statBlock(label: "On list", value: "\(items.count)", color: PrecisionCalMacroAutopsyTheme.terracotta)
                Divider().frame(height: 36).background(PrecisionCalMacroAutopsyTheme.glassStroke)
                statBlock(label: "To buy", value: "\(buyCount)", color: PrecisionCalMacroAutopsyTheme.sage)
                Divider().frame(height: 36).background(PrecisionCalMacroAutopsyTheme.glassStroke)
                statBlock(label: "Keep", value: "\(keepCount)", color: PrecisionCalMacroAutopsyTheme.fatColor)
            }
            .padding(.vertical, 16)
        }
    }
    ...
    private func addChip(icon: String, title: String) -> some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(PrecisionCalMacroAutopsyTheme.cardFill)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
        ...
        Button {
            ...
            .background {
                RoundedRectangle(cornerRadius: 22, style: .continuous)
                    ...
                    .shadow(color: PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.35), radius: 14, x: 0, y: 6)
            }
        }
        .buttonStyle(.plain)
    }
        ...
        Button(action: onToggle) {
            GlassCard(cornerRadius: 18) {
                HStack(spacing: 12) {
                    ZStack {
                        Circle()
                            .fill(item.keepOnList ? PrecisionCalMacroAutopsyTheme.fatColor.opacity(0.22) : PrecisionCalMacroAutopsyTheme.sage.opacity(0.18))
                            .frame(width: 38, height: 38)
                        Image(systemName: item.keepOnList ? "exclamationmark.circle.fill" : "circle")
                            .font(.system(size: 17, weight: .bold))
                            .foregroundStyle(item.keepOnList ? PrecisionCalMacroAutopsyTheme.fatColor : PrecisionCalMacroAutopsyTheme.textTertiary)
                    }
                        ...
                        HStack(spacing: 6) {
                            if !item.brand.isEmpty {
                                Text(item.brand)
                                    ...
                                    .foregroundStyle(PrecisionCalMacroAutopsyTheme.textSecondary)
                                Text("•").foregroundStyle(PrecisionCalMacroAutopsyTheme.textTertiary)
                            }
                            if item.keepOnList {
    ...
    private func pantryRow(_ p: ScannedProduct, isSelected: Bool, alreadyOnList: Bool) -> some View {
        GlassCard(cornerRadius: 18) {
            HStack(spacing: 12) {
                ZStack {
                    Circle()
                        .fill((isSelected || alreadyOnList) ? PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.18) : PrecisionCalMacroAutopsyTheme.sage.opacity(0.14))
                        .frame(width: 38, height: 38)
                    Image(systemName: alreadyOnList ? "checkmark" : (isSelected ? "checkmark" : "shippingbox.fill"))
                        .font(.system(size: 16, weight: .bold))
                        .foregroundStyle((isSelected || alreadyOnList) ? PrecisionCalMacroAutopsyTheme.terracotta : PrecisionCalMacroAutopsyTheme.sage)
                }
                VStack(alignment: .leading, spacing: 3) {
