...
struct SanctuaryPostCard: View {
    ...
    var body: some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 22, style: .continuous)
                        ...
                        .stroke(
                            LinearGradient(
                                colors: [
                                    ...
                                    PrecisionCalMacroAutopsyTheme.glassStroke.opacity(0.7),
                                ],
                ...
                .shadow(color: PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.06), radius: 18, x: 0, y: 8)
        }
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ZStack {
                Circle()
                    .fill(PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.15))
    ...
    private var reportedBadge: some View {
        ...
        .background(
            Capsule()
                .fill(
                    LinearGradient(
                        colors: [
                            PrecisionCalMacroAutopsyTheme.terracotta,
                            PrecisionCalMacroAutopsyTheme.terracottaDeep,
                        ],
                        startPoint: .topLeading, endPoint: .bottomTrailing
                    )
                )
        ...
        .shadow(color: PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.4), radius: 4, x: 0, y: 2)
    }
    ...
    private var kindBadge: some View {
        ...
        switch post.kind {
        case .bloom:
            label = "BLOOM"; icon = "circle.hexagongrid.fill"; color = PrecisionCalMacroAutopsyTheme.sage
        case .mealAnalysis:
            label = "PHD"; icon = "leaf.fill"; color = PrecisionCalMacroAutopsyTheme.terracotta
        case .encouragement:
            label = "VOICE"; icon = "quote.bubble.fill"; color = PrecisionCalMacroAutopsyTheme.fatColor
        }
    ...
    private var bloomBody: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 12) {
                miniStat("Hydration", post.hydrationProgress, PrecisionCalMacroAutopsyTheme.hydrationColor)
                miniStat("Macros", post.macroProgress, PrecisionCalMacroAutopsyTheme.carbColor)
                miniStat("Adherence", post.adherenceProgress, PrecisionCalMacroAutopsyTheme.terracotta)
            }
    ...
    private var mealBody: some View {
        VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if post.lipidSheen {
                            ...
                            .background(Capsule().fill(PrecisionCalMacroAutopsyTheme.fatColor.opacity(0.15)))
                        }
                    }
                }
                Spacer(minLength: 0)
            ...
            HStack(spacing: 0) {
                macroChip("cal", "\(Int(post.calories))", PrecisionCalMacroAutopsyTheme.terracotta)
                macroChip("P", "\(Int(post.protein))g", PrecisionCalMacroAutopsyTheme.proteinColor)
                macroChip("C", "\(Int(post.carbs))g", PrecisionCalMacroAutopsyTheme.carbColor)
                macroChip("F", "\(Int(post.fat))g", PrecisionCalMacroAutopsyTheme.fatColor)
            }
            .padding(10)
            .background {
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .stroke(PrecisionCalMacroAutopsyTheme.glassStroke.opacity(0.4), lineWidth: 1))
            }
    ...
    private var scoreBadge: some View {
        let color: Color = post.mealScore >= 80
            ? PrecisionCalMacroAutopsyTheme.sage
            : (post.mealScore >= 60 ? PrecisionCalMacroAutopsyTheme.fatColor : PrecisionCalMacroAutopsyTheme.terracotta)
    ...
    private var footer: some View {
        HStack(spacing: 16) {
            ...
            Button {
                ...
                .foregroundStyle(post.userReported ? Color.white : PrecisionCalMacroAutopsyTheme.terracottaDeep.opacity(0.75))
                .padding(.horizontal, 9).padding(.vertical, 5)
                .background(
                    Capsule()
                        .fill(post.userReported ? PrecisionCalMacroAutopsyTheme.terracotta : PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.08))
                        .overlay(Capsule().stroke(PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.3), lineWidth: 0.8))
                )
            }
            .buttonStyle(.plain)
            ...
            if post.state == .approved {
                stewardBadge("Steward · cleared", color: PrecisionCalMacroAutopsyTheme.sage, icon: "checkmark.seal.fill")
            } else if post.state == .reviewing {
                stewardBadge("Steward reviewing", color: PrecisionCalMacroAutopsyTheme.textTertiary, icon: "hourglass")
            } else {
                stewardBadge("In review queue", color: PrecisionCalMacroAutopsyTheme.terracottaDeep, icon: "exclamationmark.triangle.fill")
            }
        }
    }
    ...
    private func sendReportEmail(reason: String) {
        ...
        —
        Sent from PrecisionCalMacroAutopsy in-app report flow.
        """
