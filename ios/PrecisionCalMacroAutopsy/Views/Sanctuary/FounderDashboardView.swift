...
struct FounderDashboardView: View {
    ...
    private var statsRow: some View {
        HStack(spacing: 12) {
            stat("Posts read", value: "\(posts.count)", color: PrecisionCalMacroAutopsyTheme.sage)
            Button {
            ...
            .buttonStyle(.plain)
            stat("Pending signals", value: "\(pending.count)", color: PrecisionCalMacroAutopsyTheme.fatColor)
        }
    }
    ...
    private var queueStat: some View {
        ...
        return VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 6) {
                Text("\(queueCount)")
                    ...
                    .foregroundStyle(hasFlags ? alarm : PrecisionCalMacroAutopsyTheme.textPrimary)
                if hasFlags {
            ...
            Text("REVIEW QUEUE")
                ...
                .tracking(1.5)
                .foregroundStyle(hasFlags ? alarm : PrecisionCalMacroAutopsyTheme.textTertiary)
        }
        ...
        .background {
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        .stroke(hasFlags ? alarm.opacity(0.6) : PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.35),
    ...
    private var regenerateButton: some View {
        Button {
            ...
            HStack(spacing: 8) {
                if running {
                    ProgressView().tint(PrecisionCalMacroAutopsyTheme.terracotta).controlSize(.small)
            ...
            .background {
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.5), lineWidth: 1.2)
    ...
    private var softWaveOverlay: some View {
        Canvas { ctx, size in
            for i in 0..<3 {
                ...
                ctx.stroke(path,
                           with: .color(PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.05 - Double(i) * 0.012)),
                           lineWidth: 1)
            }
        }
    }
...
private struct InsightCard: View {
    ...
    private var priorityColor: Color {
        ...
        case "low": PrecisionCalMacroAutopsyTheme.textTertiary
        default: PrecisionCalMacroAutopsyTheme.fatColor
        }
    }
            ...
            HStack(spacing: 8) {
                ...
                Button {
                    ...
                    Image(systemName: insight.acknowledged ? "checkmark.circle.fill" : "circle")
                        .font(.system(size: 18))
                        .foregroundStyle(insight.acknowledged ? PrecisionCalMacroAutopsyTheme.sage : PrecisionCalMacroAutopsyTheme.textTertiary)
                }
                .buttonStyle(.plain)
            }
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ForEach(insight.sourceQuotes.prefix(3), id: \.self) { q in
                        HStack(alignment: .top, spacing: 6) {
                            Rectangle()
                                .fill(PrecisionCalMacroAutopsyTheme.terracotta.opacity(0.5))
                                .frame(width: 2)
                            Text("“\(q)”")
