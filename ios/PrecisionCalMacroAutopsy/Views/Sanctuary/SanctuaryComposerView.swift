...
struct SanctuaryComposerView: View {
    ...
    private func kindButton(_ k: SanctuaryPostKind, _ label: String, _ icon: String) -> some View {
        Button {
            ...
            .padding(.vertical, 14)
            .foregroundStyle(kind == k ? .white : PrecisionCalMacroAutopsyTheme.terracotta)
            .background {
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(kind == k
                          ? AnyShapeStyle(LinearGradient(colors: [PrecisionCalMacroAutopsyTheme.terracotta, PrecisionCalMacroAutopsyTheme.terracottaDeep], startPoint: .topLeading, endPoint: .bottomTrailing))
                          : AnyShapeStyle(Color.white.opacity(0.5)))
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .stroke(PrecisionCalMacroAutopsyTheme.terracotta.opacity(kind == k ? 0 : 0.4), lineWidth: 1)
                    )
            }
        }
        .buttonStyle(.plain)
            ...
            ZStack(alignment: .topLeading) {
                if bodyText.isEmpty {
                    Text(placeholder)
                        ...
                        .foregroundStyle(PrecisionCalMacroAutopsyTheme.textTertiary.opacity(0.7))
                        .padding(14)
                }
    ...
    private var bloomPreview: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 14) {
                statBubble("Hydration", hydrationProgress, PrecisionCalMacroAutopsyTheme.hydrationColor)
                statBubble("Macros", macroProgress, PrecisionCalMacroAutopsyTheme.carbColor)
                statBubble("Adherence", adherenceProgress, PrecisionCalMacroAutopsyTheme.terracotta)
            }
                ...
                ForEach(["Steady", "Energized", "Resting", "Re-centering"], id: \.self) { m in
                    Button {
                        ...
                        Text(m)
                            ...
                            .foregroundStyle(mood == m ? .white : PrecisionCalMacroAutopsyTheme.textSecondary)
                            .padding(.horizontal, 11).padding(.vertical, 6)
                            .background {
                                Capsule().fill(mood == m ? PrecisionCalMacroAutopsyTheme.terracotta : Color.white.opacity(0.5))
                                    .overlay(Capsule().stroke(PrecisionCalMacroAutopsyTheme.glassStroke, lineWidth: 0.8))
                            }
                    }
                    .buttonStyle(.plain)
                }
