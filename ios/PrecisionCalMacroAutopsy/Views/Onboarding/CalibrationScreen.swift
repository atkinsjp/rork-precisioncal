    ...
    var body: some View {
        Button(action: action) {
            Text(label)
                ...
                .foregroundStyle(isSelected ? .white : PrecisionCalMacroAutopsyTheme.textPrimary)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 12)
                .background {
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .fill(isSelected ? PrecisionCalMacroAutopsyTheme.terracotta : Color.white.opacity(0.55))
                }
                .overlay {
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .stroke(isSelected ? Color.clear : PrecisionCalMacroAutopsyTheme.glassStroke, lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
    }
            ...
            ForEach(CalibrationScreen.WeightUnit.allCases) { u in
                Button {
                    ...
                    Text(u.rawValue.uppercased())
                        ...
                        .tracking(1.5)
                        .foregroundStyle(unit == u ? .white : PrecisionCalMacroAutopsyTheme.textSecondary)
                        .frame(width: 30, height: 22)
                        .background {
                            if unit == u {
                                Capsule().fill(PrecisionCalMacroAutopsyTheme.terracotta)
                            }
                        }
                }
                .buttonStyle(.plain)
